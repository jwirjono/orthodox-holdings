      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {PROCESS_STEPS.map((step) => (
            <div
              key={step.number}
              className="bg-neutral-900 border border-neutral-800 p-8 flex flex-col justify-between hover:border-neutral-600 transition-all group rounded-xs"
            >
              <div>
                <span className="text-5xl font-light font-mono text-neutral-600 group-hover:text-white transition-colors block mb-6">
                  {step.number}
                </span>
        ...
        {/* CTA Banner */}
        <div className="mt-16 bg-neutral-900 border border-neutral-800 p-8 rounded-xs flex flex-col sm:flex-row items-center justify-between gap-6">
          <div>
            <h3 className="text-xl font-light text-white">
              Ready to execute the 4-step process for your business?
            </h3>
            <p className="text-xs text-neutral-400 font-light mt-1">
              Start Phase 01 Understand with an Orthodox senior advisory partner.
            </p>
          </div>

          <button
            onClick={() => onOpenConsultation('Halo Orthodox Holdings, saya ingin memulai Phase 01 Understand.')}
            className="px-6 py-3 bg-white text-black font-semibold text-xs uppercase tracking-widest hover:bg-neutral-200 transition-all shrink-0 flex items-center gap-2 rounded-xs"
          >
